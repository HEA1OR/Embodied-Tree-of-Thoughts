...
def inference(image, slider, mode, alpha, label_mode, anno_mode, *args, **kwargs):
    ...
    with torch.autocast(device_type='cuda', dtype=torch.float16):
        ...
        if model_name == 'semantic-sam':
            model = model_semsam
            output, mask = inference_semsam_m2m_auto.inference_semsam_m2m_auto1(model, image['image'], level, text, text_part, text_thresh, text_size, hole_scale, island_scale, semantic, label_mode=label_mode, alpha=alpha, anno_mode=anno_mode, *args, **kwargs)
