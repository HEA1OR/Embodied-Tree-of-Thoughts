...
def objective(opt_vars,
    ...
    for control_point_homo in control_points_homo:
        ...
        if ik_result.success:
            reset_reg = np.linalg.norm(ik_result.cspace_position[:-1] - np.asarray(reset_joint_pos[:-1]))
...
class PathSolver:
    ...
    def _warmup(self):
        ...
        self.solve(start_pose, end_pose, sdf_voxels, collision_points, None, from_scratch=True)
        self.last_opt_result = None
    ...
    # = other setup
    # ====================================
        collision_points_centered = collision_points
        aux_args = (og_bounds,
                    start_pose,
                    end_pose,
                    sdf_func,
