...
def quat_multiply(quaternion1, quaternion0):
    ...
    dtype = np.asarray(quaternion1).dtype 
    return np.array(
        (
...
def quat_conjugate(quaternion):
    ...
    """
    dtype = np.asarray(quaternion).dtype 
    return np.array(
...
def pose2mat(pose):
    ...
    pos, orn = pose
    dtype = np.asarray(pos).dtype        # 一定拿到 np.dtype
    homo_pose_mat = np.eye(4, dtype=dtype)
    homo_pose_mat[:3, :3] = quat2mat(orn).astype(dtype)
    homo_pose_mat[:3, 3]  = np.asarray(pos, dtype=dtype)
